    ...
    public class TagService : ITagService
    {
        ...
        public async Task CreateTag(TagRequestDTO tagDTO, UserDTO user)
        {
            try
            {
                ...
                await adminActionNotificationHelper.CreateNotification(user, EAdminActionType.Create, tag, null);
            }
            catch (Exception ex)
            {
        ...
        public async Task DeleteTag(Guid id, UserDTO user)
        {
            try
            {
                ...
                await adminActionNotificationHelper.CreateNotification(user, EAdminActionType.Delete, null, tag);
            }
            catch (Exception ex)
            {
                if (ex.InnerException is PostgresException pgEx && pgEx.SqlState == "23505")
        ...
        public async Task UpdateTag(Guid tagId, TagRequestDTO tagDTO, UserDTO user)
        {
            try
            {
                ...
                await adminActionNotificationHelper.CreateNotification(user, EAdminActionType.Update, newTag, tag);
            }
            catch (Exception ex)
            {
                if (ex.InnerException is PostgresException pgEx && pgEx.SqlState == "23505")
