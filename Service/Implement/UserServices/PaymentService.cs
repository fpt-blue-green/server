...
namespace Service
{
	public class PaymentService : IPaymentService
	{
		...
		private async Task<bool> CheckPaymentStatus(Guid paymentId)
		{
			try
			{
				var ApiKey = _envService.GetEnv("CASSOAPI_KEY");
				...
				using (HttpClient client = new HttpClient())
				{
					...
					if (response.IsSuccessStatusCode)
					{
						...
						var matchingRecord = parsedJson["data"]?["records"]?.FirstOrDefault(r => r?["description"]?.ToString()?.Contains(paymentCode) == true);
						...
						if (matchingRecord != null)
						{
							...
							{
								throw new InvalidOperationException("Số tiền chưa đúng với số tiền đã request!");
							}
						}
					}
					return false;
